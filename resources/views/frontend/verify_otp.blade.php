							...
							<div class="ms-auto">
								<nav aria-label="breadcrumb">
									<ol class="breadcrumb mb-0 p-0">
										...
										<li class="breadcrumb-item"><a href="{{route('signup')}}">Registration</a>
										</li>
							...
							<div class="card forgot-box">
								<div class="card-body">
									@if(Session::has('message'))
										<p class="text-danger py-2">{{Session::get('message')}}</p>
									@endif
									<form method="post" action="{{route('matchotp')}}">
										@csrf
										<div class="p-4 rounded  border">
											<div class="text-center">
												<img src="{{url('frontend/images/icons/forgot-2.png')}}" width="120" alt="" />
											</div>
											<h4 class="mt-5 font-weight-bold">Verify Otp</h4>
											<p class="">Enter Otp Sent to your email <span class="text-danger">{{Session::get('email')}}</span></p>
											<div class="my-4">
												<input type="text" class="form-control form-control-lg" name="emailotp" placeholder="OTP" />
												<p class="text-danger m-0">{{$errors->first('emailotp')}}</p>
											</div>
											<p class="">Enter Otp Sent to your Mobile <span class="text-danger">{{Session::get('mobile')}}</span></p>
											<div class="my-4">
												<input type="text" class="form-control form-control-lg" name="mobileotp" placeholder="OTP" />
											</div>
