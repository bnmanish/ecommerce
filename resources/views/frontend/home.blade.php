            ...
            <div class="col-12">
                <div class="product__section--inner">
                    <div class="row row-cols-xl-4 row-cols-lg-4 row-cols-md-3 row-cols-2 mb--n30">
                        @foreach($popularProduct as $popularProductRow)
                        <div class="col mb-30">
                            <div class="product__items ">
                                <div class="product__items--thumbnail">
                                    <a class="product__items--link" href="{{route('products.details',$popularProductRow->slug)}}">
                                        @foreach($popularProductRow->images as $key => $image)
                                        <img class="product__items--img product__primary--img" src="{{url('uploads/product/'.$image->image)}}" alt="{{$popularProductRow->title}}">
                                        @if($key == 0)
                                            @php break; @endphp
                                        @endif
                                        @endforeach
                                    ...
                                    <div class="product__badge">
                                        <span class="product__badge--items sale">Popular</span>
                                    </div>
                                </div>
                                <div class="product__items--content text-center">
                                    <h3 class="product__items--content__title h4"><a href="{{route('products.details',$popularProductRow->slug)}}">{{$popularProductRow->title}}</a></h3>
                                    <div class="product__items--price">
                                        @if($popularProductRow->discount_price > 0)
                                            <span class="current__price">$ {{$popularProductRow->discount_price}}</span>
                                            <span class="old__price">$ {{$popularProductRow->price}}</span>
                                        @else
                                            <span class="old__price">$ {{$popularProductRow->price}}</span>
                                        @endif
                                    </div>
                    ...
                    <div class="product__section--wrapper">
                        ...
                        <div class="product__section--inner">
                            <div class="row row-cols-xxl-4 row-cols-xl-3 row-cols-lg-4 row-cols-md-3 row-cols-2 mb--n25">
                                @foreach($newProduct as $newProductRow)
                                <div class="col mb-25">
                                    <div class="product__items">
                                        <div class="product__items--thumbnail">
                                            <a class="product__items--link" href="{{route('products.details',$newProductRow->slug)}}">
                                                @foreach($newProductRow->images as $key => $image)
                                                <img class="product__items--img product__primary--img" src="{{url('uploads/product/'.$image->image)}}" alt="{{$newProductRow->title}}">
                                                @if($key == 0)
                                                    @php break; @endphp
                                                @endif
                                                @endforeach
                                        ...
                                        <div class="product__items--content style2 position__relative">
                                            ...
                                            <h3 class="product__items--content__title h4"><a href="{{route('products.details',$newProductRow->slug)}}">{{$newProductRow->title}}</a></h3>
                                            <div class="product__items--price style2">
                                                @if($newProductRow->discount_price > 0)
                                                    <span class="current__price">$ {{$newProductRow->discount_price}}</span>
                                                    <span class="old__price">$ {{$newProductRow->price}}</span>
                                                @else
                                                    <span class="old__price">$ {{$newProductRow->price}}</span>
                                                @endif
                                            </div>
