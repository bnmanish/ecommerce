        ...
        <div id="page-wrapper" class="gray-bg">
            ...
            <div class="row wrapper border-bottom white-bg page-heading">
                ...
                <div class="col-lg-6 text-right">
                    <a class="btn btn-sm btn-primary mt-4" href="{{route('add.product')}}"><i class="fa fa-plus"></i> Add New</a>
                </div>
            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover dataTables-example" >
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="gradeX">
                                            <td>{{$i++}}</td>
                                            <td>{{$productRow->product}}</td>
                                            <td>{{$productRow->price}}</td>
                                            <td>{{$productRow->category}}</td>
                                            <td>{{$productRow->sub_category}}</td>
                                            <td><img src="{{url('uploads/product/'.$productRow->image)}}" width="50px;"></td>
                                            <td><input id="{{$productRow->id}}" class="form-control sequence" type="text" name="sequence" style="width: 100px;" value="{{$productRow->sequence}}"></td>
                                            <td>
                                            ...
                                            <td>
                                                <a class="btn btn-xs btn-primary" href="{{route('edit.product',$productRow->id)}}"><i class="fa fa-edit"></i></a>
                                                <a class="btn btn-xs btn-danger" href="{{route('delete.product',$productRow->id)}}" onclick="return confirm('Sure! You want to delete this item!')"><i class="fa fa-trash"></i></a>
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
